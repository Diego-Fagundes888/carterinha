...
export default function SimpleStudentCard({ student, example = false, miniVersion = false }: StudentCardProps) {
  ...
  const toggleCard = () => {
    ...
    console.log("Toggling card:", showFront ? "Showing back" : "Showing front");
  };

  return (
    <div className={`${miniVersion ? 'w-full max-w-[300px] h-[600px]' : 'w-full max-w-[400px] h-[650px]'} mx-auto flex items-center justify-center`}>
      {/* Cartão frontal */}
      {showFront && (
        <div 
          className="w-full h-full bg-gradient-to-br from-background to-muted rounded-xl shadow-lg border border-border overflow-hidden cursor-pointer shine-effect"
          onClick={toggleCard}
        >
      ...
      {!showFront && (
        <div 
          className="w-full h-full bg-white rounded-xl shadow-lg border border-border overflow-hidden cursor-pointer shine-effect"
          onClick={toggleCard}
        >
          <div className="p-4 sm:p-8 h-full flex flex-col">
            {/* Header */}
